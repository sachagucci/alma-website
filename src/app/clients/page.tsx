...
export default async function PatientsPage({ searchParams }: { searchParams: { q?: string } }) {
    ...
    const patients = await getClients(query)
            ...
            <div className="max-w-7xl mx-auto px-6 py-10">
                ...
                <div className="flex justify-between items-center mb-12">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1 font-medium">Search and manage client history.</p>
                    </div>
                </div>
                ...
                <div className="grid gap-4">
                    {patients.map((p: any) => (
                        <Link key={p.patient_phone} href={`/clients/${encodeURIComponent(p.patient_phone)}`}>
                            <div className="bg-white border border-gray-200 p-6 rounded-2xl hover:shadow-lg transition-all flex items-center justify-between group">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 flex items-center justify-center text-gray-500 group-hover:bg-black group-hover:text-white transition-colors rounded-xl">
